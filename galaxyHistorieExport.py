...
def export_history(p_gi, p_history, p_user, p_update, p_warning):
    path = p_user['username'] + "/" + p_history['id'] + '.export.tar.gz'
    if p_update and not os.path.isfile(path) or not p_update:
        try:
            ...
            f = open(p_user['username'] + "/" + p_history['id'] + '.error.txt', 'w')
